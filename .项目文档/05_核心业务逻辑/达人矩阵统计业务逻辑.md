...
> 本文为口径唯一来源（Single Source of Truth）：区间边界、占比分母、总互动量与分享定义、总计行规则等仅在此维护。
...
**计算公式**：
```
达人占比 = (该层级达人数量 / 分层分母达人总数) × 100%
```

**分层分母达人总数**：
- 统计被计入“知名KOL层 + 所有自定义层级”的不重复达人总数（按 `BloggerId` 去重）
- 不包含未匹配到任何层级的达人（保证各分层达人占比之和为 100%）

**实现逻辑**：
```typescript
// 伪代码
const bloggersInLevels = new Set<string>();
levels.forEach(level => {
  level.bloggers.forEach(b => bloggersInLevels.add(b.BloggerId));
});
const totalBloggers = bloggersInLevels.size;
...
const totalInteraction = levelNotes.reduce((sum, note) => {
  return sum + note.LikedCount + note.CollectedCount + note.CommentsCount;
}, 0);
```
...
**计算公式**：
```
互动量占比 = (该层级总互动量 / 分层分母总互动量) × 100%
```

**分层分母总互动量**：
- 仅统计“知名KOL层 + 所有自定义层级”覆盖到的达人对应笔记的互动量之和（不含分享）
- 未分层达人的笔记互动量不计入分母（保证各分层互动占比之和为 100%）

**实现逻辑**：
```typescript
// 伪代码
const bloggerIdsInLevels = new Set<string>();
levels.forEach(level => level.bloggers.forEach(b => bloggerIdsInLevels.add(b.BloggerId)));
const totalInteraction = notes
  .filter(note => bloggerIdsInLevels.has(note.BloggerId))
  .reduce((sum, note) => sum + note.LikedCount + note.CollectedCount + note.CommentsCount, 0);
...
**说明**：
- 分享量不计入“总互动量”，单独作为一列展示与统计占比
- 分享占比计算基准为“报告所有有效笔记的分享量之和”

**实现逻辑**：
```typescript
// 伪代码
const levelShare = levelNotes.reduce((sum, note) => sum + note.ShareCount, 0);
const totalShare = notes.reduce((sum, note) => sum + note.ShareCount, 0);
const sharePercentage = totalShare === 0 ? 0 : (levelShare / totalShare) * 100;
```
...
1. **后端计算**：
   - 达人矩阵所有统计（分层、达人数、平均粉丝、总互动量、占比、笔记数、分享等）由后端接口统一计算并返回
   - 前端仅负责展示与导出 Excel，不在前端重复计算
2. **缓存策略**：
   - 配置与统计结果可按报告维度做短期缓存，降低重复计算压力
...
3. **计算优化**：
   - 使用聚合函数和分组减少内存搬运
   - 分层匹配在 SQL/内存中采用一次遍历完成优先级匹配（KOL → 自定义层）
