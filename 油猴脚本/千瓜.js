...
#${BRAND_UPDATE_MODAL_ID} .az-brand-start-btn[disabled] {
    ...
    async function handleButtonClick(e) {
        ...
        if (window.currentBrandQueryParams) {
            ...
            try {
                ...
                console.log('[song DateCoverage] 保存失败:', saveResult.error);
    ...
    var domObserver = new MutationObserver(function(mutations) {
        ...
        console.log('[AZ按钮插入] DOM变化检测，检查插入位置...');
    ...
    function insertButtonsIfNeeded() {
        // 定义两个插入位置
        var xpath1 = "//div[contains(@class,'list-container b-list qg-common-list')]//span[@class='el-pagination__jump'][contains(text(),'前往')]";
        var xpath2 = "//div[contains(@class,'list-con note-list qg-scrollo-list scroll-mode')]/..//span[@class='el-pagination__jump'][contains(text(),'前往')]";

        // 查找位置1
        var targetElement1 = document.evaluate(xpath1, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
        // 查找位置2
        var targetElement2 = document.evaluate(xpath2, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;

        console.log('[AZ按钮插入] 位置1存在:', !!targetElement1, '位置2存在:', !!targetElement2);

        // 位置1：检查并插入
        if (targetElement1) {
            var existingBtn1 = document.evaluate(
                xpath1 + "/../button[contains(text(),'AZ_推送')]",
                document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
            ).singleNodeValue;

            if (!existingBtn1) {
                console.log('[AZ按钮插入] 位置1 - 插入按钮');
                var btn1 = button.cloneNode(true); // 克隆按钮
                btn1.addEventListener('click', handleButtonClick); // 绑定事件 🔥
                targetElement1.parentNode.insertBefore(btn1, targetElement1.nextSibling);
                updateBtnName(window.savedCnt || 0, window.totalCount || 0);
            } else {
                console.log('[AZ按钮插入] 位置1 - 按钮已存在');
            }
        }

        // 位置2：检查并插入
        if (targetElement2) {
            var existingBtn2 = document.evaluate(
                xpath2 + "/../button[contains(text(),'AZ_推送')]",
                document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
            ).singleNodeValue;

            if (!existingBtn2) {
                console.log('[AZ按钮插入] 位置2 - 插入按钮');
                var btn2 = button.cloneNode(true); // 克隆按钮
                btn2.addEventListener('click', handleButtonClick); // 绑定事件 🔥
                targetElement2.parentNode.insertBefore(btn2, targetElement2.nextSibling);
                updateBtnName(window.savedCnt || 0, window.totalCount || 0);
            } else {
                console.log('[AZ按钮插入] 位置2 - 按钮已存在');
            }
        }
    }

    // 页面加载后初始化
    console.log('[AZ按钮插入] 初始化监听器...');
    setTimeout(function() {
        console.log('[AZ按钮插入] 执行初始按钮插入检查...');
        insertButtonsIfNeeded();

        // 启动DOM监听
        domObserver.observe(document.body, observerConfig);
        console.log('[AZ按钮插入] ✅ DOM监听已启动');
    }, 1000);

    function updateBtnName(savedCnt, totalCnt) {
        // 获取所有 AZ_推送 按钮（可能有多个，分别在不同的tab内容中）
        var buttons = document.querySelectorAll('button');
        var azButtons = Array.from(buttons).filter(function(btn) {
            return btn.textContent.includes('AZ_推送');
        });

        console.log('[AZ按钮更新] 找到', azButtons.length, '个按钮需要更新');

        if (azButtons.length === 0) {
            return; // 没有按钮，退出
        }
        ...
        // 更新所有按钮
        azButtons.forEach(function(buttonElement) {
            // 更新按钮文本
            ...
            if (enabled) {
                ...
                buttonElement.style.backgroundColor = '#1890ff';
                buttonElement.style.cursor = 'pointer';
                buttonElement.style.opacity = '1';
            } else {
                buttonElement.disabled = true;
                buttonElement.style.backgroundColor = '#d9d9d9';
    ...
    XMLHttpRequest.prototype.send = function(data) {
        ...
        this.addEventListener('load', function() {
            // 监控商业笔记接口（支持两个接口，数据结构完全相同，统一处理）
            // 接口1: GetNoteBloggerNotes - 品牌博主笔记列表
            // 接口2: GetBrandBusinessNoteList - 品牌商业笔记列表
            if (this._url.includes('//api.qian-gua.com/v2/brand/GetNoteBloggerNotes') ||
                this._url.includes('//api.qian-gua.com/v2/brand/GetBrandBusinessNoteList')) {
                // 保存数据到全局变量
                var params = JSON.parse(this._requestPayload);
                const pageindex = params.pageindex;

                // 获取当前的签名值（包含接口标识，确保不同接口的数据不混合）
                var currentSignature = payloadSignatureWithApi(this._requestPayload, this._url);
    ...
    // 带接口标识的签名函数（用于区分不同接口的数据）
    function payloadSignatureWithApi(requestPayLoad, apiUrl) {
        // 提取接口路径（去除query参数和域名）
        // 例如："//api.qian-gua.com/v2/brand/GetNoteBloggerNotes?xxx"
        //   → "/v2/brand/GetNoteBloggerNotes"
        var apiPath = apiUrl.split('?')[0].replace(/^.*\/\/[^/]+/, '');

        // 获取原有的参数签名
        var paramSignature = payloadSignature(requestPayLoad);

        // 组合：接口路径 + '_' + 参数签名
        // 这样确保不同接口即使参数相同，签名也不同
        return apiPath + '_' + paramSignature;
    }
